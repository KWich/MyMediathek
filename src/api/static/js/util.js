 ...
 */
export function utilSendHttpRequest(type, url, load, usejsontype, funcsuccess, funcerr, addheader) {
  let xhttp = new XMLHttpRequest();
  xhttp.open(type, url, true);
  //xhttp.timeout = cfgHTTPRequestTimeout;
  if (usejsontype) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4) {
      ...
      if (this.status == 200 || this.status == 204) {
        ...
        let detail;
        if (this.response) {
          let result = utilJsonParse(this.response);
          if (result) {
            detail = result.status + " " + result.title + ": " + result.detail ? JSON.stringify(result.detail) : " - ";
          }
          else {
            detail = this.response.status + " " + this.response.title + ": " + this.response.detail ? this.response.detail : " - ";
          }
        }
        else {
          detail = this.status + " - " + this.statusText;
        }
        utilViewPopUp("Netzwerkfehler: " + detail.replaceAll(/[<>]/g, ""), "err");
      }
    }
  };
