...
function deleteUnusedCat() {
  ...
  util.utilSendHttpRequest("DELETE", gBookmarkServerUrl + "/api/categories/" + name, null, false, nb > 1 ? deleteUnusedCat : triggerReload);
}


function requestPlayerInfo() {
  util.utilSendHttpRequest("GET", gBookmarkServerUrl + "/api/info", null, false, requestPlayerInfoSuccess);
}
  ...
  if (result) {
    ...
    if (result.players.length > 0) {
      for (let i = 0; i < result.players.length; i++) {
        ...
        let ts = "Typ: \"" + result.players[i].type + "\", Adresse: \"" + result.players[i].address + ":" + result.players[i].port + "\"";
...
function loadStatisticsData() {
  ...
  util.utilSendHttpRequest("GET", gBookmarkServerUrl + "/api/info/statistics?daysbefore=" + gDaysBefore, null, false, loadStatsSuccess);
}
...
function savePlayerConfig() {
  ...
  util.utilSendHttpRequest("POST", gBookmarkServerUrl + "/api/player", JSON.stringify(data), true, savePlayerConfigResponse, savePlayerConfigResponseError);
  util.utilSetWait(true);
}
...
function deletePlayerConfigCard(event) {
  let idx = event.currentTarget.parentNode.getAttribute("idx");
  util.utilSendHttpRequest("DELETE", gBookmarkServerUrl + "/api/player/" + idx, null, false, savePlayerConfigResponse, deleteResponseError);
}
...
function testPlayerConfig() {
  ...
  util.utilSendHttpRequest("GET", gBookmarkServerUrl + "/api/player/" + idx + "/test", null, false, testPlayerConfigResponse, savePlayerConfigResponseError);
}
...
function saveFiltersFromDialog() {
  ...
  util.utilSendHttpRequest("PATCH", gBookmarkServerUrl + "/api/info/config", JSON.stringify(config), true, patchFilterSuccess, patchFilterError);
}
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  document.getElementById("deluncat").onclick = function () {
    deleteUnusedCat();
  };

  // Delete old moviestate
  document.getElementById("deloldmstate").onclick = function () {
    util.utilSendHttpRequest("DELETE", gBookmarkServerUrl + "/api/moviestate?daysbefore=" + gDaysBefore, null, false, triggerReload);
  };
