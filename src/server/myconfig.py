...
class MyConfig(ConfigParser):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
  ...
  def __init__(self, baseDir):
    ...
    self._configFile = os.environ["BM_CONFIG_FILE"] if "BM_CONFIG_FILE" in os.environ else os.path.join(self._baseDir.parent,"data","server.ini")
    if os.path.exists(self._configFile):
      super().read(self._configFile, None)
      self._updatePlayerList()
    else:
      print("NO Ini file provided, using default values:")
  ...
  def getPlayerInfoJSON(self):
    ...
    if self._chromeCastName is not None:
      players.append( { "idx": 'cc', "name" : self._chromeCastDisplayName, "type" : "Chromecast", "address" : self._chromeCastName , "port" : -1})
    return players
  ...
  def getPlayerInfo(self):
    ...
    if pc > 0:
      if pc > 1:
        pinfo = f" * {pc} Player sind konfiguriert:"
      else:
        pinfo = " * ein Player ist konfiguriert:"
      pinfo += ("\n   Id:        Name:                      Adresse:                       Typ:")
  ...
  def addPlayer(self, address, port, ptype, name, auth, default):
  ...
  def saveConfig2File(self):
    ...
    with open(self._configFile, 'w', encoding="utf-8") as configfile:
  ...
  def logLevel(self):
    ...
    return lmap[ll] if ll in lmap else logging.ERROR
