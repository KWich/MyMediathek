...
class MediaPlayer:
  ...
  def playMovie(self, title, filmurl, timestamp, playerIdx=None):
    ...
    if timestamp > self.oldTimeStamp:
      ...
      result = (
        500,
        f"Zeitstempel {timestamp!s} ist älter als gespeicherter Zeitstempel ({self.oldTimeStamp!s}) => Film wird nicht noch mal abgespielt!",  # noqa: E501
      )
    return result
  ...
  def playWithChromeCast(self, title, filmurl):
    if self._initChromeCastConnection():
      self._chromecast.playUrl(filmurl, "video/mp4", title=title)
      result = (200, "")
    else:
  ...
  def _initChromeCastConnection(self):
    ...
    if APP_CONFIG.chromeCastName is not None:
      if self._chromecast is None:
        ...
        self._chromecast = ChromeCast.getInstance(APP_CONFIG.chromeCastName, self._cbChromecastStatusChange)
        time.sleep(1)
  ...
  def playWithVLC(self, title, filmurl, playeraddress, pauth):
    ...
    try:
      ...
      if r.status_code == RETURN_CODE_OK:
