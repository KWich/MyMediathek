...
CONST_VERSION = "1.0.4 (10/02/2025)"
CONST_DBVERSION = 3


def printInfo():
  print(
    f" * Config Datei       : {APP_CONFIG.configFile}\n"
    + f" * Database Datei     : {APP_CONFIG.dbName}\n"
    + f" * Basisverzeichnis   : {APP_CONFIG.baseDir}\n"
    + f" * Server IP Addresse : {APP_CONFIG.serverIpAddress}\n\n"
    + " * Debugmodus         : "
    + ("aktiv" if APP_CONFIG.getboolean("develop", "enable_debug_mode", fallback=False) else "inaktiv")
    + "\n * Server SSL         : "
    + ("aktiv" if APP_CONFIG.sslEnabled else "inaktiv")
  )
...
connex_app = connexion.App(
  __name__,
  server_args={"template_folder": APP_CONFIG.templateDir, "static_folder": APP_CONFIG.staticDir},
  specification_dir=basedir,
  options={"swagger_ui": APP_CONFIG.getboolean("develop", "enable_swagger_ui", fallback=False)},
)
